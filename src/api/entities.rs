//! Author: TechnoL0g
//!
//! `/api/entities*` — AIP-36 entities as `core-magistrate-api` serves them:
//! `{ id, address, publicKey, isResigned, type, subType, data: { name, ipfsData } }`.
...
fn entity_json(st: &Shared, id: &str, owner: &str, rec: &EntityRecord) -> Value {
    ...
    json!({
        ...
        "data": { "name": rec.data.name, "ipfsData": rec.data.ipfs_data },
    })
}
...
fn all(st: &Shared) -> Result<Vec<Value>, crate::error::Error> {
    let mut items: Vec<Value> = st.storage.all_entities()?.iter().map(|(id, owner, rec)| entity_json(st, id, owner, rec)).collect();
...
pub async fn list(State(st): State<Shared>, Query(q): Query<Params>) -> ApiResult {
    ...
    Ok(Json(paginate("/entities", &q, total, page)))
}

pub async fn by_id(State(st): State<Shared>, Path(id): Path<String>) -> ApiResult {
    let (owner, rec) = st.storage.get_entity(&id)?.ok_or_else(|| not_found("Entity"))?;
    Ok(Json(json!({ "data": entity_json(&st, &id, &owner, &rec) })))
}
...
pub async fn search(State(st): State<Shared>, Query(q): Query<Params>, Json(body): Json<Value>) -> ApiResult {
    ...
    let (total, page) = slice_page(items, &q);
    Ok(Json(paginate("/entities/search", &q, total, page)))
}
